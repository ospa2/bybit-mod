...
export async function loadAndDisplayReviews(originalAd: Ad) {
    ...
    try {
        ...
        if (balanceValueEl) {
            balanceValueEl.textContent = (currentBalance|| 0).toString() + " USDT";
        }
        ...
        if (balanceElement) {
            balanceElement.textContent = `${currentBalance.toLocaleString('ru-RU', {minimumFractionDigits: 4, maximumFractionDigits: 4})} ${originalAd.tokenId || 'USDT'}`;
        }
        ...
        if(availableBalanceElement) {
            availableBalanceElement.textContent = `Доступно для ${originalAd.side === 1 ? 'покупки' : 'продажи'}: ${currentBalance.toLocaleString('ru-RU', {minimumFractionDigits: 4})} ${originalAd.tokenId || 'USDT'}`;
        }
